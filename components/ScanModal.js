...
// Scan captures/remembers people. It does NOT confirm attendance or absence.
// The first successful real scan completes the Scan onboarding experience.
...
export default function ScanModal({isOpen,onClose}){
  ...
  // Mark Scan onboarding complete only after the real scan succeeds.
  const completeScanOnboarding=useCallback(async()=>{
    ...
    try{
      ...
      if(!response.ok)throw new Error('Unable to save Scan onboarding progress');
      onboarding.completeExperience('scan');
    }catch(error){
      console.error('[ONBOARDING] Scan completion error:',error);
    }
  },[onboarding]);
  ...
  const startPolling=useCallback((jobId)=>{
    ...
    pollRef.current=setInterval(async()=>{
      ...
      try{
        ...
        if(!res.ok){
          console.error('[SCAN] Polling error:',await res.text());
          return;
        }
        ...
        if(data.status==='processing'||data.status==='pending'||data.status==='retrying'){
          ...
          const msgs={
            ...
            matching_community:'ARIA is comparing with your people…',
          ...
          // Only a completed successful scan finishes Scan onboarding.
    ...
    <div className="scan-modal-overlay" onClick={handleClose}>
        ...
        <div style={{maxWidth:600,margin:'0 auto',padding:'0 20px',textAlign:'center'}}>
          ...
          {stage==='processing'&&(
            <div className="fiducia-card" style={{padding:24,marginTop:20,textAlign:'center'}}>
              ...
              <p className="aria-speaks" style={{fontSize:16,marginBottom:8}}>
                {progressMessage}
              </p>
              ...
              <p style={{color:'rgba(255,255,255,0.3)',fontSize:12,marginTop:8}}>
                You can leave this page — ARIA will keep working.
              </p>
            </div>
          )}
              ...
              {(revealedPeople||[]).map((p,i)=>(
                <div key={i} style={{
                  ...
                  <span style={{
                    ...
                    padding:'2px 8px',
                    borderRadius:12,
                    background:p.relationship_stage==='new_visitor'
                      ?'rgba(52,211,153,0.15)'
                      :p.relationship_stage==='familiar_face'||p.relationship_stage==='returning'
                        ?'rgba(212,175,55,0.15)'
                        :'rgba(96,165,250,0.15)',
                    color:p.relationship_stage==='new_visitor'
                      ?'#34D399'
                      :p.relationship_stage==='familiar_face'||p.relationship_stage==='returning'
                        ?'#D4AF37'
                        :'#60A5FA'
                  }}>
                    {p.relationship_stage==='new_visitor'
                      ?'New visitor'
                      :p.relationship_stage==='familiar_face'
                        ?'Familiar face'
                        :p.relationship_stage==='returning'
                          ?'Returning'
                          :'Regular'}
                  </span>
          ...
          {stage==='complete'&&summary&&(
            ...
            >
              ...
              <p style={{color:'#f0f0f0',fontSize:18,marginBottom:16}}>
                {summary.total} people processed.
              </p>
              ...
              <div style={{
                ...
                {summary.newVisitors>0&&(
                  <div style={{color:'#34D399',fontSize:15}}>
                    {summary.newVisitors} first-time visitors
                  </div>
                )}

                {summary.returning>0&&(
                  <div style={{color:'#D4AF37',fontSize:15}}>
                    {summary.returning} recognised people
                  </div>
                )}

                {summary.duplicates>0&&(
                  <div style={{color:'#F59E0B',fontSize:15}}>
                    {summary.duplicates} existing matches detected
                  </div>
                )}

                {summary.needsReview>0&&(
                  <div style={{color:'#EF4444',fontSize:15}}>
                    {summary.needsReview} need your attention
                  </div>
                )}
              </div>

              <p className="aria-speaks" style={{marginBottom:20,fontSize:14}}>
                ARIA has finished preparing your people memory. Attendance has not been recorded by this scan.
              </p>

              <button
                onClick={()=>router.push('/people?tab=community')}
          ...
          {stage==='error'&&(
            <div className="fiducia-card" style={{padding:24,marginTop:20,textAlign:'left'}}>
              <p style={{color:'#EF4444',marginBottom:12}}>
                {message||'ARIA was unable to read the register. Please try again with a clearer photo.'}
              </p>

              <button
                onClick={()=>updateState({stage:'idle'})}
      ...
      <style jsx>{`
        ...
        @keyframes celebratePulse{
          ...
          50%{box-shadow:0 0 30px 10px rgba(212,175,55,.15)}
        ...
        .fiducia-card{
          ...
          border-radius:26px;
          border:1px solid rgba(255,255,255,.05);
          box-shadow:inset 0 0 10px rgba(212,175,55,.03);
        ...
        @keyframes cardBreathe{
          0%{box-shadow:inset 0 0 10px rgba(212,175,55,.03)}
          100%{box-shadow:inset 0 0 14px rgba(212,175,55,.06)}
        }
        .fiducia-button{
          padding:12px 24px;
          border-radius:30px;

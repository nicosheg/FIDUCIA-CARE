...
// Large, compact attendance modal. Uses the authenticated attendance APIs.
...
export default function AttendanceModal({isOpen,onClose}){
 ...
 const load=useCallback(async()=>{
  ...
  try{
   ...
   const r=await fetch('/api/attendance/active-session',{headers:h});
   ...
   <div style={S.modal}>
    <div style={S.head}>
     <div>
      ...
      <div style={S.sub}>{session?.name||'Loading...'}</div>
     </div>
     <button onClick={onClose} style={S.close}>×</button>
    </div>

    <div style={S.hint}>Tap a person when you see them.</div>

    {error&&(
     <div style={S.error}>
      <span>{error}</span>
      <button onClick={load} style={S.retry}>Try again</button>
     </div>
    )}

    {loading?<div style={S.center}>Loading people...</div>:!session?
     <div style={S.center}>No active attendance session.</div>:
     <>
      <div style={S.stats}>
       <b>{marked.size}</b><span>present</span><span>•</span><b>{people.length}</b><span>people</span>
      </div>
       ...
       {visible.length?visible.map(p=>{
        ...
        return(
         <button
          ...
          style={{...S.person,...(is?S.present:{})}}
         >
          <span style={S.info}>
           <strong>{p.first_name} {p.last_name||''}</strong>
           {p.phone&&<small>{p.phone}</small>}
          </span>
          <span style={S.check}>{saving===p.id?'…':is?'✓':'○'}</span>
         </button>
        );
...
const S={
 ...
 title:{margin:0,color:'#f5f5f5',fontSize:26},
 sub:{color:'rgba(255,255,255,.45)',marginTop:3,fontSize:14},
 close:{background:'none',border:0,color:'#aaa',fontSize:32,cursor:'pointer',lineHeight:1},
 hint:{padding:'10px 24px 2px',color:'rgba(255,255,255,.45)',fontSize:14},
 ...
 person:{minHeight:58,padding:'10px 13px',borderRadius:13,border:'1px solid rgba(255,255,255,.07)',background:'rgba(255,255,255,.035)',color:'#eee',display:'flex',alignItems:'center',justifyContent:'space-between',textAlign:'left',cursor:'pointer'},
 present:{background:'rgba(76,175,80,.13)',border:'1px solid rgba(76,175,80,.5)'},
 info:{display:'flex',flexDirection:'column',gap:3},
 check:{fontSize:22,color:'#4CAF50',marginLeft:8},

// components/AttendanceModal.js — FIDUCIA CARE: Global live attendance modal.
...
export default function AttendanceModal({isOpen,onClose}){
  ...
  const channel=supabase
   ...
   .on('postgres_changes',{
    ...
    setPeople(current=>current.map(p=>{
     if(p.id!==row.people_id)return p;

     if(payload.eventType==='DELETE'){
      return{...p,marked:false,marked_by_name:null};
     }

     return{
      ...p,
      marked:row.present===true,
      marked_by_name:row.present===true
       ?(row.marked_by_name||p.marked_by_name||'Another user')
       :null
     };
    }));
   })
   .subscribe(status=>{
    if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'){
     console.error('[ATTENDANCE] Realtime channel error:',status);
    }
   });
 ...
 const createSession=async()=>{
  ...
  if(!name){
   setError('Enter a name for this attendance session.');
   return;
  }
  ...
  try{
   ...
   setPeople(current=>current.map(p=>
    p.id===id
     ?{
       ...p,
       marked:d.present===true,
       marked_by_name:d.present===true
        ?(d.marked_by_name||p.marked_by_name||'You')
        :null
      }
     :p
   ));
 ...
 const leaveSession=async()=>{
  ...
  const confirmed=window.confirm(
   'Leave this attendance session?\n\nThis will permanently discard this live session and all of its attendance marks. This cannot be undone.'
  );
  if(!confirmed)return;

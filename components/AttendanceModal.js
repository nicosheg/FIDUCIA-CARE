...
// FIDUCIA CARE — Live global attendance modal.
// Attendance updates are optimistic: UI changes immediately, API runs in background.
// A short sync loop keeps all ushers' attendance views aligned.
...
export default function AttendanceModal({isOpen,onClose}){
 ...
 const mounted=useRef(true),syncing=useRef(false);
 ...
 const load=useCallback(async(silent=false)=>{
  if(syncing.current&&!silent)return;
  ...
  try{
   const s=await auth();
   if(!s){
    if(mounted.current)setError('You must be logged in.');
    return;
   }
   ...
   if(mounted.current)setSession(sd);

   const pr=await fetch(
    `/api/attendance/people?session_id=${encodeURIComponent(sd.session_id)}`,
    {headers:h}
   );
   ...
   if(mounted.current)setPeople(Array.isArray(pd)?pd:[]);
  }catch(e){
   console.error('[ATTENDANCE]',e);
   if(!silent&&mounted.current)setError(e.message||'Could not load attendance.');
  }finally{
 ...
 useEffect(()=>{
  ...
  // Global fallback sync. Keeps different ushers' screens aligned.
  const interval=setInterval(()=>load(true),2500);
  return()=>clearInterval(interval);
 },[isOpen,load]);

 // Supabase Realtime accelerates global updates when attendance_records
 // is enabled for Realtime. The polling above remains the safety fallback.
 useEffect(()=>{
 ...
 const updateAttendance=async(id,nextPresent)=>{
  if(!session||saving)return;

  // Optimistic update — user sees the result immediately.
  const previous=people;
  setError('');
  setPeople(p=>p.map(x=>x.id===id?{
   ...x,
   marked:nextPresent,
   marked_by_name:nextPresent?'You':null
  }:x));
  ...
  try{
   ...
   // Don't block the UI. Refresh silently so server state wins.
   load(true);
  }catch(e){
   console.error('[ATTENDANCE] Background update failed:',e);

   // Roll back only if the database rejected the action.
   if(mounted.current)setPeople(previous);

   if(mounted.current)setError(
    e.message||'Attendance could not be saved. Your change was undone.'
   );
  }
 };
 ...
 >
  <div className="attendance-modal" role="dialog" aria-modal="true" aria-label="Live attendance">
   ...
   <header>
    <div className="heading">
     ...
     <p>
      {session
       ?'Everyone is seeing the same live attendance.'
   ...
   !session?<div className="empty session-empty">
    ...
    <div className="people">
     ...
     >
      ...
      <button
       ...
       disabled={false}
       onClick={()=>updateAttendance(p.id,!p.marked)}
      >
       {p.marked
        ?<><span>✓</span> Present</>
        :'Mark present'}
  ...
  <style jsx>{`
   ...
   .mark-button{flex-shrink:0;border:1px solid rgba(255,255,255,.1);border-radius:999px;padding:8px 11px;background:rgba(255,255,255,.08);color:rgba(255,255,255,.87);font-size:11px;font-weight:650;cursor:pointer;white-space:nowrap;transition:.18s}.mark-button:hover{background:rgba(212,175,55,.16);border-color:rgba(212,175,55,.32);color:#f4d77a;transform:translateY(-1px)}.mark-button.done{background:rgba(76,175,80,.14);border-color:rgba(76,175,80,.22);color:#a9dcae}

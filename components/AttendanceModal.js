...
// Flow: Create → Mark → Keep session (close/save) or Leave.
// Closed sessions are finalized; no unsave/undo after Keep session.
...
export default function AttendanceModal({isOpen,onClose}){
 const[session,setSession]=useState(null),[people,setPeople]=useState([]),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState(''),[query,setQuery]=useState(''),[confirmLeave,setConfirmLeave]=useState(false),[confirmKeep,setConfirmKeep]=useState(false);
 ...
 const keepSession=async()=>{
  ...
  try{
   ...
   if(!r.ok||!d.success)throw Error(d.error||'Could not save attendance session.');
   setConfirmKeep(false);
   setSession(null);
   setPeople([]);
   onClose();
  }catch(e){console.error('[ATTENDANCE] Close error:',e);setError(e.message||'Could not save attendance session.');setConfirmKeep(false)}
  finally{setSaving(false)}
 };
 ...
 useEffect(()=>{
  ...
  const esc=e=>{
   if(e.key==='Escape'){
    ...
    else if(confirmKeep)setConfirmKeep(false);
    else onClose();
   }
  };
  document.addEventListener('keydown',esc);
  return()=>document.removeEventListener('keydown',esc);
 },[isOpen,onClose,confirmLeave,confirmKeep]);
 ...
 const content=<div className="attendance-overlay" onMouseDown={e=>{if(e.target===e.currentTarget&&!saving)onClose()}}>
  <div className="attendance-modal" role="dialog" aria-modal="true" aria-label="Live attendance">
   <header>
    ...
    <button className="close" onClick={()=>!saving&&setConfirmLeave(true)} aria-label="Leave attendance">×</button>
   </header>
    ...
    <>
     ...
     <footer>
      ...
      <div className="footer-actions">
       <button className="leave-button" disabled={saving} onClick={()=>setConfirmLeave(true)}>Leave</button>
       <button className="keep-button" disabled={saving} onClick={()=>setConfirmKeep(true)}>{saving?'Saving...':'Keep session'}</button>
      </div>
     </footer>
    </>
   }

   {confirmLeave&&<div className="confirm-overlay">
    <div className="confirm-box">
     <div className="confirm-icon">×</div>
     <strong>Leave this session?</strong>
     <p>Your current attendance markings will remain in the active session, but the session will not be closed or saved as completed.</p>
     <div className="confirm-actions">
      <button onClick={()=>setConfirmLeave(false)}>Keep session</button>
      <button className="danger" onClick={leaveSession}>Leave</button>
     </div>
    </div>
   </div>}

   {confirmKeep&&<div className="confirm-overlay">
    <div className="confirm-box">
     <div className="confirm-icon success">✓</div>
     <strong>Keep this session?</strong>
     <p>{present} {present===1?'person is':'people are'} marked present. Keeping the session will close and save the attendance.</p>
     <div className="confirm-actions">
      <button onClick={()=>setConfirmKeep(false)}>Cancel</button>
      <button className="success-button" disabled={saving} onClick={keepSession}>{saving?'Saving...':'Keep session'}</button>
     </div>
    </div>
   </div>}
  </div>
  ...
  <style jsx>{`
   ...
   footer{flex-shrink:0;min-height:52px;box-sizing:border-box;padding:8px 28px;border-top:1px solid rgba(255,255,255,.075);display:flex;align-items:center;justify-content:space-between;color:rgba(255,255,255,.38);font-size:11px}.live{display:flex;align-items:center;gap:7px}.live-dot{width:7px;height:7px;border-radius:50%;background:#6bd174;box-shadow:0 0 0 4px rgba(107,209,116,.08)}.footer-actions{display:flex;align-items:center;gap:8px}.leave-button,.keep-button{border-radius:999px;padding:9px 16px;font-size:11px;font-weight:700;cursor:pointer;transition:.18s}.leave-button{border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.055);color:rgba(255,255,255,.65)}.leave-button:hover:not(:disabled){background:rgba(255,255,255,.1);color:#fff}.keep-button{border:1px solid rgba(212,175,55,.35);background:rgba(212,175,55,.14);color:#f4d77a}.keep-button:hover:not(:disabled){background:rgba(212,175,55,.22)}button:disabled{opacity:.55;cursor:default}
   ...
   .confirm-overlay{position:absolute;inset:0;z-index:20;background:rgba(2,5,12,.62);backdrop-filter:blur(10px);display:flex;align-items:center;justify-content:center;padding:24px}.confirm-box{width:min(390px,90%);padding:25px;border:1px solid rgba(255,255,255,.13);border-radius:22px;background:linear-gradient(145deg,rgba(37,51,72,.97),rgba(10,17,30,.98));box-shadow:0 25px 70px rgba(0,0,0,.55);text-align:center}.confirm-icon{width:42px;height:42px;margin:0 auto 12px;border-radius:14px;background:rgba(255,255,255,.07);display:flex;align-items:center;justify-content:center;font-size:22px;color:rgba(255,255,255,.7)}.confirm-icon.success{background:rgba(76,175,80,.12);color:#9bd69f}.confirm-box strong{display:block;font-size:17px;color:#fff}.confirm-box p{margin:9px auto 20px;max-width:310px;line-height:1.5;font-size:12px;color:rgba(255,255,255,.5)}.confirm-actions{display:flex;justify-content:center;gap:8px}.confirm-actions button{border:1px solid rgba(255,255,255,.1);border-radius:999px;padding:9px 16px;background:rgba(255,255,255,.07);color:#fff;font-size:11px;font-weight:700;cursor:pointer}.confirm-actions .danger{background:rgba(239,68,68,.13);border-color:rgba(239,68,68,.25);color:#ffabab}.confirm-actions .success-button{background:rgba(76,175,80,.15);border-color:rgba(76,175,80,.28);color:#b7e8b9}

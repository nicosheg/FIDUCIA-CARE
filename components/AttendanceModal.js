...
export default function AttendanceModal({isOpen,onClose}){
 ...
 const content=<div className="attendance-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
  ...
  <style jsx>{`
   ...
   @media(max-width:720px){.attendance-overlay{padding:92px 9px 15px}.attendance-modal{width:97vw;height:78vh;min-height:500px;border-radius:24px}header{padding:19px}.hero-stats{padding:14px 19px 12px;gap:14px}.toolbar{padding:0 19px 11px}.people{padding:0 19px 12px;grid-template-columns:1fr;gap:7px}footer{padding:9px 19px}.footer-actions{gap:6px}.leave-button,.keep-button,.done-button{padding:8px 13px}}

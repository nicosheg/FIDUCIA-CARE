...
// FIDUCIA CARE — Live Attendance
// Flow: Create → Mark → Keep session → Create New Session
// Leave exits the UI without closing the active session.
...
export default function AttendanceModal({isOpen,onClose}){
 const[session,setSession]=useState(null),[people,setPeople]=useState([]),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[creating,setCreating]=useState(false),[error,setError]=useState(''),[query,setQuery]=useState(''),[newName,setNewName]=useState(''),[createView,setCreateView]=useState(false);
 ...
 const load=useCallback(async()=>{
  ...
  try{
   ...
   if(!r.ok)throw Error(d.error||'Could not load attendance.');
   if(!d.active){setSession(null);setPeople([]);setCreateView(true);return}
   setSession(d);setCreateView(false);
   const p=await fetch(`/api/attendance/people?session_id=${encodeURIComponent(d.session_id)}`,{headers:h}),pd=await p.json();
   if(!p.ok)throw Error(pd.error||'Could not load attendance people.');
   setPeople(Array.isArray(pd)?pd:[]);
  }catch(e){console.error('[ATTENDANCE]',e);setError(e.message||'Could not load attendance.')}
  finally{setLoading(false)}
 },[]);

 useEffect(()=>{if(isOpen){setCreateView(false);load()}},[isOpen,load]);

 const createSession=async()=>{
  const name=newName.trim();
  if(!name)return setError('Enter a name for this attendance session.');
  ...
  try{
   ...
   if(!r.ok||!d.success)throw Error(d.error||'Could not create attendance session.');
   setNewName('');setSession({...d.session,session_id:d.session.id,active:true});setPeople([]);setQuery('');setCreateView(false);
   // Load the people belonging to the new session.
   const p=await fetch(`/api/attendance/people?session_id=${encodeURIComponent(d.session.id)}`,{headers:{Authorization:`Bearer ${s.access_token}`}});
   const pd=await p.json();
   if(p.ok)setPeople(Array.isArray(pd)?pd:[]);
  }catch(e){console.error('[ATTENDANCE] Create error:',e);setError(e.message||'Could not create attendance session.')}
 ...
 const mark=async id=>{
  ...
  try{
   ...
   const r=await fetch('/api/attendance/mark',{
    method:'POST',
    headers:{'Content-Type':'application/json',Authorization:`Bearer ${s.access_token}`},
    body:JSON.stringify({session_id:session.session_id||session.id,people_id:id})
   }),d=await r.json();
 ...
 const keepSession=async()=>{
  ...
  try{
   ...
   setSession(null);setPeople([]);setQuery('');setCreateView(true);setError('');
  }catch(e){console.error('[ATTENDANCE] Close error:',e);setError(e.message||'Could not keep this session.')}
  finally{setSaving(false)}
 };
 ...
 const content=<div className="attendance-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
  <div className="attendance-modal" role="dialog" aria-modal="true" aria-label="Attendance">

   <header>
    <div className="heading">
     <div className="eyebrow">FIDUCIA CARE</div>
     <h2>{createView?'New attendance session':session?.name||'Attendance'}</h2>
     <p>{createView?'Create a session to begin attendance.':session?'Tap a person when you see them.':'Attendance'}</p>
    </div>
    <button className="close" onClick={onClose} aria-label="Close attendance">×</button>
   </header>

   {error&&<div className="error"><span>{error}</span><button onClick={()=>{setError('');load()}}>Try again</button></div>}
   ...
   <div className="create-view">
    ...
    <span>Give this attendance session a name, then begin marking people present.</span>
    <input value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>e.key==='Enter'&&createSession()} placeholder="e.g. Sunday Service" autoFocus/>
    <button className="primary-action" onClick={createSession} disabled={creating||!newName.trim()}>
   ...
   <>
    ...
    <footer>
     ...
     <div className="footer-actions">
      <button className="leave-button" onClick={onClose} disabled={saving}>Leave</button>
      <button className="keep-button" onClick={keepSession} disabled={saving}>{saving?'Saving…':'Keep session'}</button>
     </div>
    </footer>
   </>}
  </div>
  ...
  <style jsx>{`
   ...
   header,.error,.hero-stats,.toolbar,.people,footer,.loading,.create-view{position:relative;z-index:1}
   ...
   .toolbar{padding:0 28px 13px;flex-shrink:0}.search-wrap{position:relative}.search{width:100%;box-sizing:border-box;padding:13px 42px;border-radius:15px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.065);color:#fff;font-size:14px;outline:none}.search::placeholder{color:rgba(255,255,255,.34)}.search:focus{border-color:rgba(212,175,55,.4)}.search-icon{position:absolute;left:16px;top:50%;transform:translateY(-51%);color:rgba(255,255,255,.4);font-size:20px;z-index:1}.clear-search{position:absolute;right:9px;top:50%;transform:translateY(-50%);width:28px;height:28px;border:0;border-radius:50%;background:rgba(255,255,255,.09);color:rgba(255,255,255,.7);cursor:pointer;font-size:18px}
   ...
   .mark-button{flex-shrink:0;border:1px solid rgba(255,255,255,.1);border-radius:999px;padding:8px 11px;background:rgba(255,255,255,.08);color:rgba(255,255,255,.87);font-size:11px;font-weight:650;cursor:pointer;white-space:nowrap}.mark-button.done{background:rgba(76,175,80,.14);border-color:rgba(76,175,80,.22);color:#a9dcae}
   footer{flex-shrink:0;min-height:58px;box-sizing:border-box;padding:9px 28px;border-top:1px solid rgba(255,255,255,.075);display:flex;align-items:center;justify-content:space-between;color:rgba(255,255,255,.38);font-size:11px}.live{display:flex;align-items:center;gap:7px}.live-dot{width:7px;height:7px;border-radius:50%;background:#6bd174;box-shadow:0 0 0 4px rgba(107,209,116,.08)}.footer-actions{display:flex;gap:8px}.leave-button,.keep-button{border-radius:999px;padding:10px 17px;font-size:12px;font-weight:650;cursor:pointer}.leave-button{border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.06);color:rgba(255,255,255,.75)}.keep-button{border:1px solid rgba(212,175,55,.35);background:rgba(212,175,55,.15);color:#f4d77a}.leave-button:disabled,.keep-button:disabled{opacity:.5;cursor:default}
   .loading,.create-view{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;color:rgba(255,255,255,.4);font-size:12px}.create-view strong{color:rgba(255,255,255,.8);font-size:17px}.create-view span{max-width:340px;text-align:center;line-height:1.5}.create-icon{width:48px;height:48px;border-radius:15px;background:rgba(212,175,55,.1);color:#d4af37;display:flex;align-items:center;justify-content:center;font-size:25px}.create-view input{width:min(360px,82%);box-sizing:border-box;margin-top:8px;padding:13px 15px;border-radius:14px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.07);color:#fff;outline:none;font-size:14px}.create-view input::placeholder{color:rgba(255,255,255,.3)}.primary-action{border:1px solid rgba(212,175,55,.35);background:rgba(212,175,55,.16);color:#f4d77a;border-radius:999px;padding:11px 20px;font-weight:650;cursor:pointer}.primary-action:disabled{opacity:.45;cursor:default}.loader{width:22px;height:22px;border:2px solid rgba(255,255,255,.1);border-top-color:rgba(212,175,55,.85);border-radius:50%;animation:spin .75s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
   .empty{grid-column:1/-1;min-height:220px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:9px;color:rgba(255,255,255,.38);font-size:12px}.empty strong{color:rgba(255,255,255,.62);font-size:14px}.empty span{max-width:300px;text-align:center;line-height:1.5}.empty-icon{width:43px;height:43px;border-radius:14px;background:rgba(255,255,255,.055);display:flex;align-items:center;justify-content:center;font-size:21px;color:rgba(255,255,255,.35)}
   ...
   @media(max-width:480px){.attendance-overlay{padding:82px 6px 8px}.attendance-modal{width:98vw;height:82vh;min-height:480px;border-radius:22px}.attendance-modal header{padding:17px}.attendance-modal header p{font-size:12px}.hero-stats{padding:12px 17px 10px;gap:11px}.main-stat strong{font-size:27px}.small-stat strong{font-size:17px}.progress-wrap{min-width:50px}.toolbar{padding:0 17px 10px}.people{padding-left:17px;padding-right:17px}.person{min-height:60px;padding:8px}.avatar{width:35px;height:35px;flex-basis:35px}.mark-button{padding:7px 9px;font-size:10px}footer{padding-left:17px;padding-right:17px}.footer-actions{gap:5px}.leave-button,.keep-button{padding:9px 12px}}
  `}</style>
 </div>;

...
// Handles live marking + closing the active attendance session.
...
export default function AttendanceModal({isOpen,onClose}){
 const[session,setSession]=useState(null),[people,setPeople]=useState([]),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[closing,setClosing]=useState(false),[error,setError]=useState(''),[query,setQuery]=useState('');
 ...
 const load=useCallback(async()=>{
  ...
  try{
   ...
   if(!sd.active){
    setSession(null);setPeople([]);setQuery('');return;
   }
 ...
 const mark=async id=>{
  if(!session||saving||closing)return;
 ...
 // The API confirms present records, closes the session,
 // and starts participation generation after commit.
 const closeSession=async()=>{
  if(!session||closing||saving)return;

  const confirmed=window.confirm(
   `Close "${session.name}"?\n\nAll currently marked people will be confirmed and the attendance session will end.`
  );
  ...
  try{
   ...
   if(!r.ok||!d.success){
    throw Error(d.error||'Could not close attendance session.');
   }

   // Immediately remove the old session from the UI.
   setSession(null);
   setPeople([]);
   setQuery('');

   // Re-check the server so the UI reflects the actual active-session state.
   await load();
  }catch(e){
   console.error('[ATTENDANCE] Close session error:',e);
   setError(e.message||'Could not close attendance session.');
  }finally{
   setClosing(false);
  }
 };
 ...
 >
  <div className="attendance-modal" role="dialog" aria-modal="true" aria-label="Live attendance">
   ...
   :
    ...
    <div className="empty session-empty">
     ...
     <span>Start an attendance session before marking people present.</span>
    </div>
   :
    <>
       ...
       visible.map(p=>
        ...
        >
         ...
         <button
          ...
          disabled={p.marked||saving||closing}
          onClick={()=>mark(p.id)}
         >
          {p.marked
           ?<><span>✓</span> Present</>
  ...
  <style jsx>{`
   ...
   .close{flex-shrink:0;width:42px;height:42px;border-radius:50%;border:1px solid rgba(255,255,255,.13);background:rgba(255,255,255,.08);color:rgba(255,255,255,.85);font-size:27px;line-height:1;cursor:pointer;box-shadow:inset 0 1px 0 rgba(255,255,255,.1);transition:.18s}.close:hover:not(:disabled){background:rgba(255,255,255,.14);color:#fff;transform:scale(1.04)}.close:disabled{opacity:.5;cursor:default}
   ...
   footer{flex-shrink:0;min-height:52px;box-sizing:border-box;padding:9px 28px;border-top:1px solid rgba(255,255,255,.075);display:flex;align-items:center;justify-content:space-between;gap:15px;color:rgba(255,255,255,.38);font-size:11px}.live{display:flex;align-items:center;gap:7px}.live-dot{width:7px;height:7px;border-radius:50%;background:#6bd174;box-shadow:0 0 0 4px rgba(107,209,116,.08)}.footer-actions{display:flex;align-items:center;gap:14px}.footer-count{color:rgba(255,255,255,.3)}.close-session{border:1px solid rgba(239,68,68,.25);border-radius:999px;padding:8px 13px;background:rgba(239,68,68,.09);color:rgba(255,190,190,.9);font-size:11px;font-weight:650;cursor:pointer;white-space:nowrap;transition:.18s}.close-session:hover:not(:disabled){background:rgba(239,68,68,.16);border-color:rgba(239,68,68,.4);color:#fff;transform:translateY(-1px)}.close-session:disabled{opacity:.55;cursor:default}
   ...
   @media(max-width:720px){.attendance-overlay{padding:92px 9px 15px}.attendance-modal{width:97vw;height:78vh;min-height:500px;border-radius:24px}header{padding:19px}.hero-stats{padding:14px 19px 12px;gap:14px}.toolbar{padding:0 19px 11px}.people{padding:0 19px 12px;grid-template-columns:1fr;gap:7px}footer{padding:9px 19px}.footer-actions{gap:9px}.footer-count{display:none}}
   @media(max-width:480px){.attendance-overlay{padding:82px 6px 8px}.attendance-modal{width:98vw;height:82vh;min-height:480px;border-radius:22px}.attendance-modal header{padding:17px}.attendance-modal header p{font-size:12px}.hero-stats{padding:12px 17px 10px;gap:11px}.main-stat strong{font-size:27px}.small-stat strong{font-size:17px}.progress-wrap{min-width:50px}.toolbar{padding:0 17px 10px}.people{padding-left:17px;padding-right:17px}.person{min-height:60px;padding:8px}.avatar{width:35px;height:35px;flex-basis:35px}.mark-button{padding:7px 9px;font-size:10px}footer{padding-left:17px;padding-right:17px}.close-session{padding:7px 10px;font-size:10px}}
  `}</style>
 </div>;

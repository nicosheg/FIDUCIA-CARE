...
function extractAndParseJSON(text) {
  ...
  // 2. Remove <think>...</think> blocks (including nested)
  let prev;
  do {
    prev = cleaned;
  ...
  // 3. Trim and look for JSON structure
  cleaned = cleaned.trim();

  // Try to find a JSON object (starting with {) or array (starting with [)
  ...
  // Extract from start to the end, then try to balance braces/brackets
  ...
  try {
    ...
    // Direct parse failed – try to find balanced JSON by tracking braces
    // We'll look for the first { or [ and then balance to the last matching }
    let stack = [];
    let end = -1;
    ...
    // Final repair attempt: fix trailing commas, single quotes, etc.
...
export async function validateScanOutput(rawContent, orgId, programName, jobId, options = {}) {
  ...
  // 3. Enforce hard limit before any processing
  ...
  // 4. Zero‑person case – success with empty result
  ...
  // 7. Identity resolution for unique people

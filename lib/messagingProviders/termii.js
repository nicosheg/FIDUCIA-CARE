...
const senderId = process.env.TERMII_SENDER_ID || 'talert';   // default sender that works with generic
const baseUrl = process.env.TERMII_BASE_URL || 'https://v4.api.termii.com/api/';
...
export async function sendSMS(phone, message, options = {}) {
  ...
  const channel = options.channel || 'generic';   // generic works without sender approval

...
export async function getAttentionItems(orgId) {
  ...
  // 4. People who missed the last 2 Sundays
  ...
  for (const p of notContactedRes.rows) {
    items.push({ person_id: p.id, text: `${p.first_name} hasn't been contacted for a long time.` });
  }

  // Prayer needs
  for (const p of prayerRes.rows) {
    items.push({ person_id: p.id, text: `${p.first_name} asked for prayer over a week ago.` });
  }

  // Birthdays
  for (const p of birthdaysRes.rows) {
    items.push({ person_id: p.id, text: `${p.first_name} has a birthday this week.` });
  }

  // Absent
  for (const p of absentRes.rows) {
    items.push({ person_id: p.id, text: `${p.first_name} hasn't attended recently. May appreciate a follow-up.` });
  }

  // If nothing, return a calm message
  if (items.length === 0) {
    items.push({ person_id: null, text: 'Everyone is well taken care of today.' });
  }

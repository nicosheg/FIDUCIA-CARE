...
/**
 * Process a canonical ARIA event.
 * - Creates an observation based on the event type.
 * - Updates person state.
 * - Optionally triggers action planning.
 *
 * @param {object} event - The event row from aria_events.
...
export async function processAriaEvent(event, client = null) {
  ...
  // Only process events that affect a person
  if (!personId) {
  ...
  // Determine observation type and confidence based on event type
  ...
  switch (type) {
    ...
    case 'PERSON_UPDATED':
      ...
      break;
    // Add more event → observation mappings as we expand
    default:
  ...
  const obsId = await createObservation({
    ...
    evidence: evidence,
    expiresAt: null,
    // We pass the event id for idempotency via metadata

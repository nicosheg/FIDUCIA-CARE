...
const result=await pool.query(`
 INSERT INTO aria_learning(
  organization_id,person_id,scope_key,learning_type,learning_key,value,confidence,source_type,source_id,active,created_at,updated_at
 ...
 DO UPDATE SET
  ...
  updated_at=NOW()
 RETURNING *
`,[organizationId,personId,scopeKey,learningType,learningKey,value,safeConfidence,sourceType,sourceId]);
...
organizationId,
personId,
learningType:'care_response',
learningKey:'latest_feedback',
value:{feedback_type:feedbackType,sentiment,context,action_id:actionId},
confidence,
...
organizationId,
personId:null,
learningType:'organization_pattern',
learningKey:`feedback:${feedbackType}`,
value:{feedback_type:feedbackType,sentiment,context,person_id:personId},
confidence:.5,
sourceType:'human_feedback',
sourceId:feedbackId
});
...
organizationId,
personId:null,
learningType:'voice_signal',
learningKey:positive?'positive_response':'negative_response',
value:{feedback_type:feedbackType,sentiment},
confidence:.6,
sourceType:'human_feedback',
sourceId:feedbackId
});
...
organizationId,
personId:null,
learningType:'action_outcome',
learningKey:`${outcome}`,
value:{action_id:actionId,person_id:personId,outcome,outcome_score:score,evidence},
...
where+=` AND(person_id=$${params.length} OR person_id IS NULL)`;
}
if(learningType){
params.push(learningType);
where+=` AND learning_type=$${params.length}`;
}

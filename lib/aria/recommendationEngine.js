...
export async function executeAction(actionId,organizationId){
 ...
 if(action.type==='SEND_MESSAGE'){
  await pool.query(`UPDATE aria_actions SET outcome=$1,updated_at=NOW() WHERE id=$2 AND organization_id=$3`,[{success:true,requiresHumanSend:true,message:'Care draft is ready for human sending.'},actionId,organizationId]);
  return getAction(actionId,organizationId);
 }
 await pool.query(`UPDATE aria_actions SET status='executed',executed_at=NOW(),outcome=$1,updated_at=NOW() WHERE id=$2 AND organization_id=$3`,[{success:true},actionId,organizationId]);
 return getAction(actionId,organizationId);
}

async function getAction(id,orgId){
 const r=await pool.query(`SELECT * FROM aria_actions WHERE id=$1 AND organization_id=$2 LIMIT 1`,[id,orgId]);
 return r.rows[0]||null;
}

...
export async function initializeCommunity(orgId) {
  ...
  // ── Step 3: Run all observers and log counts ──
 ...
 * Groups evidence by (person_a, person_b) using string ordering for UUIDs.
...
async function processPairObservations(orgId, observations) {
  ...
  for (const obs of observations) {
    if (!obs.person_id) {
      console.warn('[ARIA] Skipping observation with missing person_id:', obs);
      continue;
    }
    ...
    for (const otherId of otherIds) {
      if (!otherId) {
        console.warn('[ARIA] Skipping observation with missing matched_person_id:', obs);
        continue;
      }

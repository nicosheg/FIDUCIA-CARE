...
export async function initializeCommunity(orgId) {
  ...
  // LOG 1: Similar Name Observations
  const similarNameObs = await generateSimilarNameObservations(orgId);
  console.log('[ARIA DEBUG] Similar Name Observations:', similarNameObs.length);
  if (similarNameObs.length > 0) {
    const first20 = similarNameObs.slice(0, 20);
    first20.forEach((obs, idx) => {
      console.log(`  [${idx+1}] person_id=${obs.person_id}, type=${obs.type}, confidence=${obs.confidence}, matched=${obs.evidence.matched_person_id}`);
    });
  }

  // LOG 2: Similar Phone Observations
  const similarPhoneObs = await generatePhoneSimilarityObservations(orgId);
  console.log('[ARIA DEBUG] Similar Phone Observations:', similarPhoneObs.length);
  if (similarPhoneObs.length > 0) {
    const first20 = similarPhoneObs.slice(0, 20);
    first20.forEach((obs, idx) => {
      console.log(`  [${idx+1}] person_id=${obs.person_id}, type=${obs.type}, confidence=${obs.confidence}, matched=${obs.evidence.matched_person_id}`);
    });
  }

  // Also get duplicate phone observations (but we may not need to log all of them)
  const duplicatePhoneObs = await generateDuplicatePhoneObservations(orgId);
  console.log('[ARIA DEBUG] Duplicate Phone Observations:', duplicatePhoneObs.length);
  ...
  // LOG 3: Total Observations
  console.log('[ARIA DEBUG] Total Observations:', allObservations.length);
...
async function processPairObservations(orgId, observations) {
  ...
  // LOG 4: After pairMap creation
  console.log('[ARIA DEBUG] Pair Count:', pairMap.size);
  if (pairMap.size > 0) {
    console.log('[ARIA DEBUG] Pairs:');
    for (const [key, pair] of pairMap) {
      const obsTypes = pair.observations.map(o => o.type).join(', ');
      const confs = pair.observations.map(o => o.confidence).join(', ');
      console.log(`  ${pair.person_a} ↔ ${pair.person_b}: types=[${obsTypes}], conf=[${confs}]`);
    }
  }
  ...
  try {
    ...
    for (const [key, pair] of pairMap) {
      ...
      // LOG 5: Before eligibility check
      console.log(`[ARIA DEBUG] Eligibility: person_a=${person_a}, ltA_status=${ltA?.status || 'null'}, source=${ltA?.source || 'null'}; person_b=${person_b}, ltB_status=${ltB?.status || 'null'}, source=${ltB?.source || 'null'}`);

      if (!ltA || !ltB) {
        console.log(`[ARIA DEBUG] Skipping pair ${person_a} ↔ ${person_b}: missing living_truth`);
        continue;
      }
      ...
      if (!isEligible(ltA) || !isEligible(ltB)) {
        console.log(`[ARIA DEBUG] Skipping pair ${person_a} ↔ ${person_b}: not eligible (already resolved or conflict)`);
        continue;
      }
      ...
      // LOG 6: Before score threshold
      console.log(`[ARIA DEBUG] Pair Score: ${person_a} ↔ ${person_b}, score=${combinedScore.toFixed(0)}, types=${Array.from(typeSet).join(', ')}, count=${count}`);

      // If score < 85, skip
      if (combinedScore < 85) {
        console.log(`[ARIA DEBUG] Skipping pair ${person_a} ↔ ${person_b}: score below threshold`);
        continue;
      }
      ...
      // LOG 7: After update
      console.log(`[ARIA DEBUG] Updated Pair: ${person_a} ↔ ${person_b}`);
    }
  ...
  // Log top 50 pair suspicions (for debugging)

...
export async function updatePersonState(personId,orgId,client=null){
 ...
 const[person,intelligence,metrics,relationship,observations,actions,engagementCase]=await Promise.all([
  ...
  db.query(`SELECT score,relationship_state FROM relationship_scores WHERE organization_id=$1 AND person_id=$2 LIMIT 1`,[orgId,personId]),
  db.query(`
   SELECT COUNT(*)::int AS count,COALESCE(MAX(attention_score),0)::float AS max_attention
  ...
  db.query(`
   ...
   FROM aria_actions
   WHERE organization_id=$1 AND person_id=$2 AND status IN('proposed','approved')
  `,[orgId,personId]),
  db.query(`
 ...
 const c=engagementCase.rows[0]||{};
 const openActions=Number(actions.rows[0]?.count)||0;

 const attentionRisk=Math.max(riskFromAttention(i.attention_score),riskFromAttention(o.max_attention));
 const engagementState=c.engagement_status||'first_time';
 const relationshipStatus=r.relationship_state||relationshipState(r.score);

 const careState=
  c.risk_level==='critical'?'urgent_action_required':
  c.risk_level==='high'?'at_risk':
  c.risk_level==='medium'?'needs_attention':
  'active';

 const followupState=i.next_best_action&&i.next_best_action!=='observe'?'recommended':'none';
 ...
 await db.query(`
  INSERT INTO aria_person_state(
   ...
   last_meaningful_event,lifecycle_state,engagement_score,churn_probability,
   next_best_action,attention_reason,updated_at

...
function attentionFromScore(score) {
  ...
  const normalized = Math.max(
    0,
    Math.min(4, Math.round(Number(score)))
  );
...
export async function updatePersonState(
  ...
  if (personRes.rows.length === 0) {
    throw new Error(
      `Person ${personId} not found in organization ${orgId}`
    );
  }
  ...
  const actionCount =
    Number(actionsRes.rows[0]?.count) || 0;

  const observationRiskLevel =
    observationRisk(
      observationStats.max_attention
    );

  const severityRiskLevel =
    Number(observationStats.max_severity) || 0;
  ...
  const engagementState =
    deriveEngagementState(events);

  const relationshipState =
    deriveRelationshipState(events);

  const careState =
    deriveCareState(highestRisk);

  const attentionLevel =
    attentionFromScore(highestRisk);

  const lastMeaningfulEvent =
    eventsRes.rows[0]?.occurred_at || null;
  ...
  await db.query(
    ...
    DO UPDATE SET
      ...
      engagement_state =
        EXCLUDED.engagement_state,
      care_state =
        EXCLUDED.care_state,
      relationship_state =
        EXCLUDED.relationship_state,
      attention_level =
        EXCLUDED.attention_level,
      open_observation_count =
        EXCLUDED.open_observation_count,
      open_action_count =
        EXCLUDED.open_action_count,
      last_meaningful_event =
        EXCLUDED.last_meaningful_event,
      updated_at = NOW()`,
    [
      personId,
      orgId,

...
const model=process.env.GROQ_CHAT_MODEL||process.env.GROQ_TEXT_MODEL||'llama-3.3-70b-versatile';
...
function nameOf(person){
 return clean(person.display_name||[person.first_name,person.last_name].filter(Boolean).join(' ')||person.first_name||'there',120);
}
...
function fallback(person,action){
 ...
 const templates={
  ...
  default:`Hi ${first}, just wanted to check in and see how you're doing. You're remembered.`
 };
...
export async function createCareDraft({organizationId,personId,actionId=null,actionType='thoughtful_check_in'}){
  ...
  try{
   ...
   const completion=await client.chat.completions.create({
    ...
    messages:[
     {
      role:'system',
      content:`You are ARIA, the care intelligence inside NYEOCARE. Write a short, warm, genuinely human WhatsApp message from an organization to one person. Use only supplied facts. Never mention monitoring, scoring, churn, risk, attendance analytics, databases, AI, intelligence, internal systems, or why ARIA selected the person. Never invent a circumstance, event, relationship, prayer request, problem, or personal detail. Do not pressure the recipient. Respect the organization's voice instructions when supplied. Keep it natural for Nigeria without forcing slang. Return ONLY the message text. Organization voice instructions: ${clean(organization.aria_instructions,1800)||'Warm, respectful, personal, sincere.'}`
     },
     {role:'user',content:JSON.stringify(facts)}
    ]
   });
   message=clean(completion.choices?.[0]?.message?.content,2000);
  }catch(err){
   console.error('[ARIA] Draft generation failed:',err.message);
  }
 }
 ...
 `,[
  ...
  message,
  {
   communication_id:communication.rows[0].id,
   action_id:actionId,
   action_type:actionType,
   requires_human_send:true,
   requires_human_approval:true

...
export async function processVisionJob(jobId, imageBase64, orgId, programName, options = {}) {
  ...
  try {
    ...
    // ── IDENTITY RESOLUTION ──
    const { needsReview, resolvedPeople } = await resolveIdentities(validation.people, orgId, jobId);

    // ── Prepare for persistence ──
    // For each person, we have either a resolved_person_id (if auto‑matched) or need to create new.
    // We'll store the mapping for later use.
    const personIdMap = {};
    const unresolvedNewPeople = [];

    for (const p of resolvedPeople) {
      if (p.resolved_person_id) {
        personIdMap[p.name] = p.resolved_person_id;
      } else {
        unresolvedNewPeople.push(p);
      }
    }

    // Now we can proceed with session/attendance creation.
    // We'll modify the existing insertion logic to use personIdMap when available.
    ...
    for (const person of validation.people) {
      ...
      const resolvedId = personIdMap[person.name];
      let pid;

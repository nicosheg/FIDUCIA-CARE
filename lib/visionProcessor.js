...
// ── Added for engagement intelligence ──
...
export async function processVisionJob(jobId, imageBase64, orgId, programName, options = {}) {
  ...
  try {
    ...
    // ── Non‑blocking engagement updates (ARIA 2) ──
    if (!evaluation) {
      // Fire and forget – updates engagement metrics and cases asynchronously
      updateEngagementMetrics(orgId).catch(err => console.error('Engagement metrics update failed:', err));
      updateEngagementCases(orgId).catch(err => console.error('Engagement cases update failed:', err));
    }

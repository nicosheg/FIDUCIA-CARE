...
export async function processVisionJob(jobId, imageBase64, orgId, programName, options = {}) {
  ...
  try {
    ...
    try {
      ...
      if (!evaluation) {
        await updateJobState(client, jobId, INTERNAL_STATES.FAILED, 'failed', { error: friendly }, 0, null, startTime);
      }
    ...
    if (!validation.valid) {
      const error = validation.error || 'ARIA could not extract valid data.';
      if (!evaluation) {
        await updateJobState(client, jobId, INTERNAL_STATES.FAILED, 'failed', { error }, attemptCount, providerUsed, startTime);
      }
      return { error };
    }
    ...
    if (validatedCount > MAX_PEOPLE_PER_SCAN) {
      const error = `Validation produced ${validatedCount} people, exceeding limit of ${MAX_PEOPLE_PER_SCAN}.`;
      console.error(error);
      if (!evaluation) {
        await updateJobState(client, jobId, INTERNAL_STATES.FAILED, 'failed', { error }, attemptCount, providerUsed, startTime);
      }
    ...
    for (const person of validation.people) {
      ...
      const insertRes = await client.query(
        `INSERT INTO people (organization_id, first_name, phone, type, status, confidence, source, created_by, last_scan_job_id)
         VALUES ($1, $2, $3, $4, $5, $6, 'scan', NULL, $7) RETURNING id`,
        [orgId, person.name, person.phone || null, type, 'active', person.confidence || 0.85, jobId]
      );
    ...
    if (registerMode === 'complete') {
      ...
      // Store total active count for result
      var allActiveIds = allActive.rows.map(r => r.id);
    }

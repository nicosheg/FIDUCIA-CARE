...
import { createObservation } from './aria/observationEngine';
  ...
  try {
    ...
    for (const person of validation.people) {
      ...
      if (resolvedItem && resolvedItem.resolved_person_id) {
        ...
        // ── ARIA: EMIT EVENT & CREATE OBSERVATION ──
        await emitAriaEvent({
        ...
        await createObservation({
          ...
          evidence: {
            sources: ['scan'],
            facts: [`Created from scan job ${jobId}`],
            inference: 'New person discovered',
          },
        });

        await updatePersonState(pid, orgId);
      }

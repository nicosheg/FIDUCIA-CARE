...
export async function processVisionJob(jobId, imageBase64, orgId, programName, options = {}) {
  ...
  let requestId = null;
  let transactionCommitted = false; // --- ADDED: track commit state
  ...
  try {
    ...
    console.log('[SCAN] Validation result:', {
      ...
      people_count: validation.people?.length || 0,
      duplicates_count: validation.duplicates?.length || 0,
      needs_review_count: validation.needsReview?.length || 0,
    });
    ...
    for (const person of validation.people) {
      ...
      if (resolvedItem && resolvedItem.resolved_person_id) {
        ...
        const rawConf = parseFloat(person.confidence);
        const conf = normalizeConfidence(rawConf, 85);
        await client.query(
          `UPDATE people SET
        ...
        const rawConf = parseFloat(person.confidence);
        const conf = normalizeConfidence(rawConf, 70);
        console.log(`[SCAN] Person: ${person.name}, rawConf: ${rawConf}, normalizedConf: ${conf}`);
        ...
        // ── ARIA: EMIT EVENT WITH CONFIDENCE IN METADATA ──
        ...
        if (event) {
          await processAriaEvent(event, client);
        }
      }
    ...
    // ── REMOVED AUTO‑ABSENCE ──

    await client.query('COMMIT');
    transactionCommitted = true; // --- ADDED: mark commit done
    ...
    // ── Mark scan COMPLETE (core transaction committed) ──
    ...
    // ── Post-commit engagement intelligence (isolated) ──
    if (!evaluation) {
      // Fire-and-forget: sequential metrics → cases
      Promise.resolve()
        .then(async () => {
          try {
            await updateEngagementMetrics(orgId);
            await updateEngagementCases(orgId);
            console.log(`[ARIA] Engagement intelligence completed for org ${orgId}`);
          } catch (intelErr) {
            // Log but never throw into scan pipeline
            console.error(`[ARIA] Engagement intelligence failed for org ${orgId}:`, intelErr);
          }
        });
    }
    ...
    if (!transactionCommitted) {
      // Core transaction not committed – rollback and mark failed
      try {
      ...
      // Transaction committed – scan is durable; just log post‑commit error
      console.error(`[SCAN] Post-commit error for ${jobId}; scan remains COMPLETE.`, err);

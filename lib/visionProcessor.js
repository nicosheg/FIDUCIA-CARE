...
export async function processVisionJob(jobId, imageBase64, orgId, programName, options = {}) {
  ...
  try {
    ...
    // ---- Get or create today's session (using organization_id) ----
    ...
    if (existingSession.rows.length > 0) {
      ...
      const newSession = await client.query(
        `INSERT INTO sessions (organization_id, name, status, created_at)
    ...
    // ---- Process duplicates (existing people) ----
    for (const dup of validation.duplicates) {
      ...
      // Mark attendance for this existing person
    ...
    // ---- Needs Review (store in result only) ----
    for (const nr of validation.needsReview) {
      needsReview.push(nr);
    }

    // ---- Insert or update new people (confident) ----
    for (const person of validation.people) {
      ...
      if (person.phone) {
        // Use advisory lock to prevent race conditions for the same phone
        const lockKey = `fiducia-person:${orgId}:${person.phone}`;
        await client.query(
        ...
        const existing = await client.query(
          `SELECT id, first_name FROM people WHERE organization_id = $1 AND phone = $2 LIMIT 1`,
          [orgId, person.phone]
        );
        if (existing.rows.length > 0) {
          ...
          await client.query(
            `UPDATE people SET
              ...
              updated_at = NOW()
             WHERE id = $3`,
            [person.confidence || 0.85, jobId, pid]
          );
        ...
        // No phone → insert as new (no conflict possible)
        const insertRes = await client.query(
          `INSERT INTO people (organization_id, first_name, phone, type, status, confidence, source, created_by, last_scan_job_id)
    ...
    if (registerMode === 'complete') {
      ...
      for (const row of allActive.rows) {
        if (!savedPeople.includes(row.id)) {
          // Insert absent record using the unique (people_id, attendance_date) constraint
          await client.query(
...
// ---- Helper function to mark attendance safely using the existing unique constraint ----

...
export async function processVisionJob(jobId, imageBase64, orgId, programName, options = {}) {
  ...
  try {
    ...
    // Observability metrics update removed – table does not exist in production
    ...
    // ---- Get or create today's session (without session_date column) ----
    const today = new Date().toISOString().slice(0, 10);
    let sessionId;

    // Check for an existing session with the same church_id, name, and today's date
    ...
    if (existingSession.rows.length > 0) {
      ...
      const newSession = await client.query(
        `INSERT INTO sessions (church_id, name, status, created_at)

...
import { validateScanOutput, normalizeName, MAX_PEOPLE_PER_SCAN } from './scanValidation';
  ...
  try {
    ...
    for (const person of validation.people) {
      ...
      if (resolvedItem && resolvedItem.resolved_person_id) {
        ...
        // ── ARIA: EMIT EVENT AND PROCESS WITHIN TRANSACTION ──
    ...
    // ── REMOVED: AUTO‑ABSENCE FROM SCAN ──
    // The following block (registerMode === 'complete') has been removed
    // because a scanned register does not represent confirmed absence.
    // Absence should only be recorded by manual marking or admin review.
    ...
    const result = {
      ...
      absent_count: undefined, // no longer computed from scan
...
async function markAttendance(client, sessionId, peopleId, sectionId, date, orgId) {
  ...
  // 2. NEW: Upsert participation_records (so engagement intelligence gets data)

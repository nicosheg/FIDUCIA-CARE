...
export async function processVisionJob(
  ...
  try {
    ...
    if (!evaluation) {
      await client.query(
        `UPDATE scan_jobs SET image_hash = $1 WHERE id = $2`,
        [hash, jobId]
      );
    }
    ...
    const sectionRes = await client.query(
      `SELECT id
       ...
       WHERE session_id = $1 AND name = 'All'`,
      [sessionId]
    );
    ...
    for (const dup of validation.duplicates) {
      ...
      if (dup.existing.first_name !== dup.incoming.name) {
        await client.query(
          `UPDATE people
           SET first_name = $1, last_scan_job_id = $2
           WHERE id = $3`,
      ...
      matchedPeople.push({
        id: existingId,
        name: dup.existing.first_name,
      });
    }

    for (const person of validation.people) {
      const resolvedItem = resolvedPeople.find(
      ...
      if (resolvedItem && resolvedItem.resolved_person_id) {
        ...
        if (
          person.relationship_stage === 'regular' ||
          person.relationship_stage === 'returning' ||
          person.relationship_stage === 'familiar_face'
        ) {
          type = 'member';
        }
        ...
        console.log(
          `[SCAN] Person: ${person.name}, rawConf: ${rawConf}, normalizedConf: ${conf}`
        );
        ...
        // Phase 5.1: deterministic PERSON_CREATED event.
    ...
    if (!evaluation) {
      Promise.resolve().then(async () => {
        try {
          ...
          console.error(
            `[ARIA] Engagement intelligence failed for org ${orgId}:`,
            intelErr
          );
        }
      });
    ...
    if (!transactionCommitted) {
      try {
        ...
        console.error(
          `Scan job ${jobId} rollback failed:`,
          rollbackErr
        );
      }

      console.error(
        `Scan job ${jobId} failed unexpectedly:`,
        err.message,
        err.stack
      );
    ...
    console.error(
      `[SCAN] Post-commit error for ${jobId}; scan remains COMPLETE.`,
      err
    );

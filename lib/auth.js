...
/**
 * Get or create a FIDUCIA user from a Supabase auth user.
 * This should be called when a user first logs in.
 */
export async function getOrCreateFiduciaUser(authUser, orgId = null) {
    ...
    // If not found, try by email (for existing users)
    result = await pool.query(
        `SELECT * FROM users WHERE email = $1`,
        [authUser.email]
    );
    if (result.rows.length > 0) {
        // Update the user with the supabase_user_id
        await pool.query(
    ...
    // If no user exists, and orgId is provided, create a new user (invitation flow)
    if (orgId) {

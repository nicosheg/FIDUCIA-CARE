...
<Section title="Groups & Memberships">{person.groups?.length?person.groups.map(x=><div key={x.id} style={row}><b>{x.name}</b><span style={muted}>{x.group_type}{x.membership_role?` · ${x.membership_role}`:''}{x.membership_status?` · ${x.membership_status}`:''}</span></div>):<Empty/>}</Section>

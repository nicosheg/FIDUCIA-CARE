...
export default withAdmin(async function handler(req,res){
 ...
 try{
  ...
  for(const personId of archivedIds){
   try{
    await client.query(`INSERT INTO timeline_events(person_id,organization_id,event_type,channel,description,metadata) VALUES($1,$2,'person_archived','manual','Person archived by an administrator.',$3)`,[
     personId,orgId,JSON.stringify({actor_id:req.user.id})
    ]);

...
export default async function handler(req,res){
  ...
  if(!authUser)return res.status(401).json({
    error:'Please sign in or create your NYEOCARE account first.'
  });
  ...
  if(!email)return res.status(400).json({
    error:'Your NYEOCARE account does not have an email address.'
  });
  ...
  try{
    ...
    if(!invite.rows.length){
      ...
      return res.status(410).json({
        error:'This invitation has expired, been used, or is no longer available.'
      });
    }
    ...
    if(i.email&&String(i.email).trim().toLowerCase()!==email){
      ...
      return res.status(403).json({
        error:`This invitation was created for ${i.email}. Please use that email address.`
      });
    }
    ...
    if(existing.rows.length){
      ...
      if(member.organization_id!==i.organization_id){
        ...
        return res.status(409).json({
          error:'Your account already belongs to another organization.'
        });
      }

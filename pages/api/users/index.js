...
}catch(err){return res.status(500).json({error:'Unable to load organization members.'})}
}
...
const invitationUrl=`${getAppUrl(req)}/join?token=${encodeURIComponent(token)}`;

return res.status(201).json({invitation:{...invitation,url:invitationUrl}});
}catch(err){return res.status(500).json({error:'Unable to create invitation.'})}
}

...
try{
const origin=process.env.NEXT_PUBLIC_SITE_URL||process.env.NEXT_PUBLIC_APP_URL||`https://${req.headers.host}`;
...
}catch(e){console.error('[PASSWORD_RESET]',e?.message||e);return res.status(500).json({error:'Unable to send the secure reset link.'})}
}

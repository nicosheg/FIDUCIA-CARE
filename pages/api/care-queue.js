...
// No engagement_cases/churn_predictions/daily_briefings dependencies.
...
function signalText(obs){
  ...
  if(evidence.inference)return evidence.inference;
  ...
  try{
    const result=await pool.query(`
      ...
      SELECT
        ...
        aps.followup_state,
        aps.attention_level,
        COALESCE(aps.open_observation_count,0) AS open_observation_count,
        COALESCE(aps.open_action_count,0) AS open_action_count,
      ...
      LEFT JOIN LATERAL(
        SELECT
          MAX(ar.attendance_date) FILTER(
            WHERE ar.present=true AND ar.confirmed=true
          ) AS last_seen,
      ...
      ORDER BY
        CASE COALESCE(a.priority,o.severity,aps.attention_level,'low')
          WHEN 'critical' THEN 4
          WHEN 'high' THEN 3
          WHEN 'medium' THEN 2
          ELSE 1
    ...
    const items=result.rows.map(row=>{
      ...
      return{
        ...
        suggestion:nextAction(row.observation_type,row.action_type?{type:row.action_type}:null),
        ...
        open_observation_count:Number(row.open_observation_count)||0,
    ...
    return res.status(500).json({error:'Unable to load ARIA care queue.'});
  }
    }

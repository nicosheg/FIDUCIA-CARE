...
async function handler(req,res){
 ...
 if(req.method==='POST'){
  ...
  try{
   const person=(await pool.query(`INSERT INTO people(organization_id,first_name,last_name,phone,email,type,birthday,created_by,living_truth,status,source) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,'active','manual') RETURNING *`,[orgId,name,String(last_name||'').trim(),normalizePhone(phone)||null,String(email||'').trim().toLowerCase()||null,String(type||'visitor').trim()||'visitor',birthday||null,req.user.id,JSON.stringify({status:'alive',confidence:90,source:'canonical_record',updated_at:new Date().toISOString()})])).rows[0];
   try{
 ...
 if(req.method==='PUT'){
  ...
  try{
   ...
   if(first_name!==undefined){const v=String(first_name).trim();if(!v)return res.status(400).json({error:'first_name cannot be empty'});if(v.length>150)return res.status(400).json({error:'Name is too long'});updates.push(`first_name=$${n++}`);values.push(v)}
   ...
   if(type!==undefined){updates.push(`type=$${n++}`);values.push(String(type||'visitor').trim()||'visitor')}
   ...
   try{
    const event=await emitAriaEvent({organizationId:orgId,personId:id,type:'PERSON_UPDATED',source:'manual',actorId:req.user.id,metadata:{updated_fields:Object.keys(req.body).filter(k=>k!=='id')},eventKey:`manual:${orgId}:person:${id}:update:${Date.now()}`});
    if(event)await processAriaEvent(event);

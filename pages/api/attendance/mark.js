...
export default async function handler(req, res) {
  ...
  const { session_id, person_id, present, usher_id, group_id } = req.body;
  ...
  try {
    ...
    await client.query(
      `INSERT INTO attendance_records (member_id, attendance_date, present, session_section_id)
       ...
       ON CONFLICT (member_id, attendance_date) DO UPDATE SET present = $3`,
      [person_id, today, present, session_id]
    );

    // Log usher mark for progressive recognition
    if (usher_id) {
      await client.query(
        `INSERT INTO usher_marks (usher_id, person_id, session_id) VALUES ($1, $2, $3)`,
        [usher_id, person_id, session_id]
      );
    }

...
// FIDUCIA CARE — Canonical global attendance endpoint.
// present=true  → mark present.
// present=false → unmark.
// UI should update optimistically; this API remains the source of truth.
...
export default withOrg(async function handler(req,res){
  ...
  const {session_id,people_id,present=true}=req.body||{};
  if(!session_id||!people_id||typeof present!=='boolean'){
    return res.status(400).json({error:'Missing or invalid session_id, people_id or present'});
  }
  ...
  try{
    ...
    // 1. Session must still be active and belong to this organization.
    ...
    // 2. Person must belong to this organization and be active.
    ...
    // 3. User must have joined the live session.
    ...
    // 4. PRESENT = upsert. UNMARK = remove the session attendance fact.
    if(present){

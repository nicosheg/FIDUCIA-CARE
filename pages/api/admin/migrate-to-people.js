...
export default async function handler(req, res) {
  ...
  try {
    // Create the people table if it doesn't exist (in case you haven't yet)
    await pool.query(`
    ...
    // Migrate existing members (skip if already migrated)
    await pool.query(`
    ...
    res.status(200).json({ message: 'Migration completed. You can now use the Community page.' });
  } catch (err) {
    console.error(err);

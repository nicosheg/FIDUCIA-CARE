...
export default async function handler(req, res) {
  if (req.method === 'GET' || req.method === 'POST') {
    try {
      // 1. Ensure people table exists (in case it was dropped)
      await pool.query(`
      ...
      // 3. Update attendance_records that might reference old member IDs
      // (just in case – the IDs should match after migration)
      await pool.query(`
      ...
      // 4. Drop the old foreign key constraint (if it exists)
      await pool.query(`
      ...
      // 6. Drop the members table completely
      await pool.query(`DROP TABLE IF EXISTS members CASCADE`);

      res.status(200).json({ message: 'Merge complete. The members table has been dropped. All data is now in people.' });
    } catch (err) {

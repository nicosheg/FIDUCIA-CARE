...
export default function JoinPage(){
  ...
  const join=async()=>{
    ...
    try{
      ...
      let session=login.data?.session;

      if(login.error){
        const signup=await supabase.auth.signUp({
          email:cleanEmail,
          password,
          options:{data:{name:data?.name||cleanEmail.split('@')[0]}}
        });
        ...
        if(signup.error){
          ...
          if(msg.includes('already registered')||msg.includes('already exists')){
            throw new Error('This email already has an account. Please use the correct password to continue.');
          }
  ...
  return(
    <div className="page">
      ...
      <main className="card">
        ...
        {state==='loading'&&(
          <div className="center">Opening invitation…</div>
        )}
        ...
        {state==='ready'&&(
          <>
            ...
            <p>
              ...
              Your NYEOCARE account remains yours.
            </p>
            ...
            >
              {working?'Joining…':'Continue'}
            </button>

            <small>
              New here? Your NYEOCARE account will be created automatically.
            </small>
          </>
        )}
        ...
        {state==='joined'&&(
          <>
            ...
            <h1>You're in.</h1>
            <p>Welcome to {data.organization_name}.</p>
          </>
        )}
      </main>
      ...
      <style jsx>{`
        *{box-sizing:border-box}
        ...
        .card{position:relative;width:100%;max-width:430px;padding:34px 26px;border:1px solid rgba(220,235,250,.14);border-radius:30px;background:rgba(17,29,47,.72);backdrop-filter:blur(24px);box-shadow:0 30px 90px rgba(0,0,0,.35);text-align:center}
        ...
        .card p{color:rgba(235,241,250,.62);line-height:1.65;margin:0 0 24px}
        .card strong{color:#f3df9c;font-weight:500}
        .card input{width:100%;height:50px;border-radius:16px;border:1px solid rgba(220,235,250,.13);background:rgba(255,255,255,.045);color:white;padding:0 15px;outline:none;margin-bottom:10px}
        .card input:disabled{opacity:.65}
        .password{position:relative}
        .password input{padding-right:65px}
        .password button{position:absolute;right:8px;top:7px;height:36px;border:0;background:transparent;color:#d4af37}
        .invite-email{font-size:13px;color:rgba(255,255,255,.45);margin:-8px 0 18px}
        .hint{font-size:12px;color:rgba(255,255,255,.35);text-align:left;margin:-3px 2px 14px}
        .error{font-size:13px;color:#ff9d9d;margin:8px 0 12px}
        .primary,.secondary{width:100%;height:50px;border-radius:18px;font-size:15px}
        ...
        .card small{display:block;margin-top:15px;color:rgba(255,255,255,.3)}
        .center{color:rgba(255,255,255,.5)}
      `}</style>
    </div>
  );
}

  ...
  useEffect(()=>{
    ...
    async function load(){
      try{
        ...
        if(active)console.error('[ARIA Today]',err);
      }finally{
    ...
    <Layout>
        ...
        {hasPattern&&(
          <section style={{marginBottom:42}}>
            ...
            {data.patterns.slice(0,3).map(p=>(
              <div key={p.id} className="fiducia-card" style={{
                padding:'18px 20px',
                marginBottom:8,
                borderRadius:22
              }}>
      ...
      {tool==='attendance'&&(
        <ActionModal
          ...
          text="Open the attendance workspace to record who was present in this session."
          button="Open Attendance"
          onClose={()=>setTool(null)}
          onAction={()=>router.push('/people?tab=attendance')}
        />
      )}

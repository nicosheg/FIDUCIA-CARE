...
export default function PeoplePage() {
  ...
  useEffect(() => {
    if (people.length > 0) setReviewItems(computeReviewItems(people));
  }, [people]);
  ...
  const generateDraft = async id => {
    ...
    if (data.message) {
      ...
      setMsg('Error: ' + (data.error || 'Draft failed'));
      setTimeout(() => setMsg(''), 3000);
    }
  };
    ...
    <Layout>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '20px' }}>
        {showPeopleExperience && (
          <FirstExperience
            experience="people"
            onComplete={() => onboarding.completeExperience('people')}
          />
        )}
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button className={`fiducia-button ${showLivingTruthOnly ? 'fiducia-button-primary' : 'fiducia-button-ghost'}`} style={{ padding: '6px 12px', fontSize: '0.8rem' }} onClick={() => setShowLivingTruthOnly(!showLivingTruthOnly)}>
        ...
        {showAdd && (
          <form onSubmit={addPerson} className="fiducia-card" style={{ padding: 20, marginBottom: 24, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <input placeholder="Phone" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} style={{ padding: '10px 12px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(20,25,40,0.6)', color: '#fff', outline: 'none' }} />
              ...
              <div key={person.id} className="fiducia-card" onPointerDown={() => onPointerDown(person.id)} onPointerUp={onPointerUp} onPointerLeave={onPointerLeave} onClick={() => { if (selectMode) toggleSelection(person.id); else if (editingId !== person.id) setSelected(selected?.id === person.id ? null : person); }} style={{ cursor: 'pointer', transition: 'all 0.2s', border: selectedIds.has(person.id) ? '1px solid #D4AF37' : undefined, background: selectedIds.has(person.id) ? 'rgba(212,175,55,0.08)' : undefined, userSelect: 'none', WebkitUserSelect: 'none', position: 'relative' }}>
                ...
                {editingId === person.id ? (
                  <div onClick={stopProp} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <input value={editName} onChange={e => setEditName(e.target.value)} style={{ padding: '10px 12px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(20,25,40,0.6)', color: '#fff', outline: 'none' }} placeholder="Full Name" />
                    <input value={editPhone} onChange={e => setEditPhone(e.target.value)} style={{ padding: '10px 12px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(20,25,40,0.6)', color: '#fff', outline: 'none' }} placeholder="Phone" />
                    <div style={{ marginBottom: 8 }}>
                  ...
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        ...
                        {status && (
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: '0.7rem', padding: '2px 10px 2px 6px', borderRadius: 20, background: 'rgba(255,255,255,0.04)', border: `1px solid ${status === 'conflict' ? 'rgba(143,183,255,0.3)' : 'rgba(255,255,255,0.08)'}`, fontWeight: 500, color: status === 'conflict' ? '#8FB7FF' : (status === 'alive' ? '#8FB7FF' : '#D4AF37'), boxShadow: `0 0 12px ${status === 'conflict' ? 'rgba(143,183,255,0.15)' : 'rgba(255,255,255,0.02)'}` }}>
                            <span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', background: statusColor(status), animation: 'pulse 2.5s ease-in-out infinite', boxShadow: `0 0 12px ${status === 'conflict' ? 'rgba(143,183,255,0.4)' : 'rgba(255,255,255,0.1)'}` }} />
                            {label}
                          </span>
                        )}
                      </div>
                    ...
                    {explanation && <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.6)', marginBottom: 6, fontStyle: 'italic' }}>{explanation}</div>}
                    ...
                    {person.last_contacted ? (
                      <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 4 }}>{ICONS.mail} Last contacted: {new Date(person.last_contacted).toLocaleDateString()}</div>
                    ) : (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                        <button onClick={e => { e.stopPropagation(); generateDraft(person.id); }} className="fiducia-button fiducia-button-primary" style={{ padding: '6px 12px', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>{ICONS.mail} Draft & Send WhatsApp</button>
                        <Link href={`/person/${person.id}`} className="fiducia-button fiducia-button-secondary" style={{ padding: '6px 12px', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>Journey →</Link>
                        <button onClick={e => { e.stopPropagation(); setSelected(person); setAddingNote(true); }} className="fiducia-button fiducia-button-ghost" style={{ padding: '6px 12px', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>{ICONS.note} Add pastoral note</button>
                        <button onClick={e => { e.stopPropagation(); setSelected(person); setImportingConv(true); }} className="fiducia-button fiducia-button-ghost" style={{ padding: '6px 12px', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>{ICONS.importIcon} Import Conversation</button>
                      </div>
                    )}

                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={e => deletePerson(person.id, e)} className="fiducia-button fiducia-button-ghost" style={{ padding: '6px 12px', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>{ICONS.trash} Remove</button>
                    </div>
                    ...
                    {addingNote && selected?.id === person.id && (
                      <div style={{ marginTop: 12, background: 'rgba(20,25,40,0.9)', borderRadius: 12, padding: 12, border: '1px solid rgba(255,255,255,0.05)' }} onClick={stopProp}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                          {[{ icon: ICONS.prayer, label: 'Asked for prayer' }, { icon: ICONS.heart, label: 'First-time visitor' }, { icon: ICONS.smile, label: 'Shared good news' }, { icon: ICONS.sick, label: 'Sick or recovering' }, { icon: ICONS.family, label: 'Family situation' }, { icon: ICONS.work, label: 'Work or school' }, { icon: ICONS.other, label: 'Other' }].map(prompt => (
                            <button key={prompt.label} onClick={e => { e.stopPropagation(); setNoteText(prompt.label + ': '); }} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', color: '#D4AF37', borderRadius: 8, padding: '6px 12px', fontSize: 12, cursor: 'pointer' }}>{prompt.icon} {prompt.label}</button>
                          ))}
                        </div>
                        <textarea value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="Add more details..." rows={3} style={{ width: '100%', padding: 8, borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.03)', color: '#fff', resize: 'vertical', outline: 'none', marginBottom: 8 }} />
                        <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    {importingConv && selected?.id === person.id && (
                      <div style={{ marginTop: 12, background: 'rgba(20,25,40,0.9)', borderRadius: 12, padding: 12, border: '1px solid rgba(255,255,255,0.05)' }} onClick={stopProp}>
                        ...
                        <textarea value={convText} onChange={e => setConvText(e.target.value)} placeholder="Paste conversation..." rows={4} style={{ width: '100%', padding: 8, borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.03)', color: '#fff', resize: 'vertical', outline: 'none', marginBottom: 8 }} />
                        <div style={{ display: 'flex', gap: 8 }}>

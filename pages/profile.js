...
if(a.ok){const d=await a.json();setProfile(d);setName(d.organization.name);setAria(d.organization.aria_instructions||'');setPersonalName(d.user.name||'')}
...
const{data:{session}}=await supabase.auth.getSession();
const body={};
if(profile.user.role==='owner'&&name!==profile.organization.name)body.name=name;
...
setProfile(p=>({...p,user:d.user||p.user,organization:d.organization}));setName(d.organization.name);setAria(d.organization.aria_instructions||'');setMsg('Saved');setTimeout(()=>setMsg(''),1800);
};

const savePersonalName=async()=>{
setSaving(true);setMsg('');
const{data:{session}}=await supabase.auth.getSession();
const r=await fetch('/api/profile',{method:'PATCH',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({userName:personalName})});
const d=await r.json();
setSaving(false);
if(!r.ok)return setMsg(d.error||'Unable to update your name.');
setProfile(p=>({...p,user:d.user||p.user}));setEditingName(false);setMsg('Your name was updated');setTimeout(()=>setMsg(''),1800);
};
...
setMsg('Secure reset link sent');
};

const remove=async id=>{
const target=users.find(x=>x.id===id);
if(!target)return;
setConfirm({type:'remove',id,title:'Remove this person?',text:`This will remove ${target.name||target.email} from your organization. They will no longer have access.`,action:'REMOVE'});
setConfirmText('');
};
...
const target=users.find(x=>x.id===id);
if(!target)return;
setConfirm({type:'transfer',id,title:'Transfer ownership?',text:`Ownership of ${profile.organization.name} will move to ${target.name||target.email}. You will immediately become an Admin.`,action:'TRANSFER OWNERSHIP'});
setConfirmText('');
};

const executeConfirm=async()=>{
if(!confirm||confirmText!==confirm.action)return;
setConfirmBusy(true);
const{data:{session}}=await supabase.auth.getSession();
if(confirm.type==='remove'){
const r=await fetch(`/api/users/${confirm.id}`,{method:'DELETE',headers:{Authorization:`Bearer ${session.access_token}`}});
const d=await r.json().catch(()=>({}));
if(!r.ok){setConfirmBusy(false);setConfirm(null);return setMsg(d.error||'Unable to remove user.')}
setConfirm(null);setConfirmBusy(false);setConfirmText('');await load();setMsg('Access removed');setTimeout(()=>setMsg(''),1800);
}else{
const r=await fetch(`/api/users/${confirm.id}`,{method:'PUT',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({role:'owner'})});
const d=await r.json().catch(()=>({}));
if(!r.ok){setConfirmBusy(false);setConfirm(null);return setMsg(d.error||'Unable to transfer ownership.')}
setConfirm(null);setConfirmBusy(false);setConfirmText('');await load();setMsg('Ownership transferred');setTimeout(()=>setMsg(''),1800);
}
};
...
<header><div className="avatar">{(u.name||'U').trim().charAt(0).toUpperCase()}</div><div className="headInfo"><div className="eyebrow">PROFILE</div>{editingName?<div className="nameEdit"><input value={personalName} onChange={e=>setPersonalName(e.target.value)} maxLength={120} autoFocus/><div className="nameActions"><button onClick={()=>{setEditingName(false);setPersonalName(u.name||'')}}>Cancel</button><button onClick={()=>{setConfirm({type:'name',title:'Update your name?',text:'Your name will be updated across NYEOCARE.',action:'UPDATE NAME'});setConfirmText('')}}>Save</button></div></div>:<><h1>{u.name}</h1><button className="editName" onClick={()=>setEditingName(true)}>Edit name</button></>}<div className="sub">{roleLabel(u.role)} · {u.email}</div></div></header>

<div className="sections">
<section className={`card ${open==='organization'?'open':''}`}><button className="cardHead" onClick={()=>setOpen(open==='organization'?null:'organization')}><div><span className="label">Organization</span><strong>{o.name}</strong><small>{users.filter(x=>x.active).length} active users · Active</small></div><span className="arrow">›</span></button>{open==='organization'&&<div className="body">{u.role==='owner'?<><label>Organization name</label><input value={name} onChange={e=>setName(e.target.value)} maxLength={120}/><button className="save" onClick={()=>{if(name.trim()===o.name)return setMsg('No organization-name change to save.');setConfirm({type:'organization',title:'Change organization name?',text:`The organization name will change from “${o.name}” to “${name.trim()}”.`,action:'CHANGE ORGANIZATION'});setConfirmText('')}}>Save organization name</button></>:<p className="hint">Only the owner can change the organization name.</p>}{u.role!=='user'&&<button className="action" onClick={()=>setOpen('users')}>Manage users ›</button>}{u.role!=='user'&&<button className="action" onClick={()=>setOpen('aria')}>ARIA knowledge ›</button>}</div>}</section>

<section className={`card ${open==='aria'?'open':''}`}><button className="cardHead" onClick={()=>setOpen(open==='aria'?null:'aria')}><div><span className="label">ARIA</span><strong>Organization knowledge</strong><small>{aria?'ARIA has context about your organization':'Tell ARIA what matters here'}</small></div><span className="arrow">›</span></button>{open==='aria'&&<div className="body"><textarea value={aria} onChange={e=>setAria(e.target.value)} disabled={!['owner','admin'].includes(u.role)} placeholder="Tell ARIA about your organization, its values, terminology, people, routines, or anything it should understand."/><div className="counter">{aria.length}/2000</div>{['owner','admin'].includes(u.role)&&<button className="save" onClick={saveOrganization} disabled={saving}>{saving?'Saving…':'Save ARIA knowledge'}</button>}</div>}</section>
...
<section className={`card ${open==='account'?'open':''}`}><button className="cardHead" onClick={()=>setOpen(open==='account'?null:'account')}><div><span className="label">ACCOUNT</span><strong>Security & preferences</strong><small>{roleLabel(u.role)} · {u.email}</small></div><span className="arrow">›</span></button>{open==='account'&&<div className="body"><button className="action" onClick={resetPassword}>Send secure reset link ›</button>{passwordEmail&&<p className="hint good">A secure reset link has been sent to {u.email}.</p>}<button className="action dangerAction" onClick={()=>{setConfirm({type:'signout',title:'Sign out of NYEOCARE?',text:'You will need to sign in again to access your organization.',action:'SIGN OUT'});setConfirmText('')}}>Sign out</button></div>}</section>
</div>

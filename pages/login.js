  ...
  return(
    <div className="auth-container">
      ...
      <div className="auth-panel">
        ...
        <form onSubmit={isLogin?handleLogin:handleSignup} className="auth-form">
          ...
          <button type="submit" disabled={loading} className="auth-button">
            {loading?(isLogin?'Signing in...':'Creating your space...'):(isLogin?'Sign In':'Create Account')}
          </button>
        </form>
        ...
        {message&&<p className={`auth-message ${message.startsWith('📨')||message.startsWith('Account created')?'success':'error'}`}>
          {message}
        </p>}
      ...
      <style jsx>{`
        ...
        .auth-button:hover{background:#E8C84A}
        ...
        .auth-magic:hover{background:rgba(255,255,255,.03)}
        .auth-message{margin-top:16px;text-align:center;font-size:14px;padding:10px 12px;border-radius:8px}
        ...
        .auth-toggle{text-align:center;color:rgba(255,255,255,.4);font-size:14px;margin-top:20px}
        .auth-toggle-link{color:#D4AF37;cursor:pointer;font-weight:500}

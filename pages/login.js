...
showMessage('Your display name is too long.');
return;
}
...
{!isLogin&&<p className="password-hint">Password must be at least 6 characters.</p>}
...
<button type="button" onClick={handleMagicLink} disabled={loading} className="auth-magic">
Send Magic Link
</button>
...
<button type="button" onClick={switchMode} className="auth-toggle-link" disabled={loading}>Create one</button>
</>
):(
<>Already have an account?{' '}
<button type="button" onClick={switchMode} className="auth-toggle-link" disabled={loading}>Log in</button>
</>
)}
</p>
</div>
...
.password-hint{margin:-5px 2px 0;color:rgba(255,255,255,.35);font-size:12px}
.auth-button{width:100%;padding:14px;border-radius:12px;border:0;background:#d4af37;color:#0a0f1a;font-weight:600;font-size:16px;cursor:pointer}

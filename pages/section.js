...
export default function SectionCheckin() {
  ...
  // Submit attendance for this section
  ...
  // Quick add new person and immediately mark present
  const handleAddPerson = async (e) => {
    ...
    try {
      ...
      // 2. Mark them present for this section/session
      ...
      // 3. Update local state: add to present set and members list
  ...
  return (
    <div style={{ padding: 20, maxWidth: 600, margin: '0 auto' }}>
      ...
      {showAddForm && (
        <form onSubmit={handleAddPerson} style={{ marginBottom: 20, background: '#f5f5f5', padding: 15, borderRadius: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="submit" disabled={adding} style={smallBtn}>
              {adding ? 'Adding...' : 'Add & Mark Present'}
            </button>
            <button type="button" onClick={() => setShowAddForm(false)} style={{ ...smallBtn, background: '#ccc' }}>
              Cancel
            </button>
          </div>
      ...
      <button
        ...
        style={{
          padding: '14px 24px',
          background: submitted ? '#aaa' : '#4CAF50',
          color: 'white',
          border: 'none',
          borderRadius: 8,
          cursor: 'pointer',
